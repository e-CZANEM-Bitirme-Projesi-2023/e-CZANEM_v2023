...
export default function LoginScreen({ navigation }) {
  ...
  const [password, setPassword] = useState({ value: "", error: "" });
   ...
   const onLoginPressed  = async () => {
    ...
    var resultFromDb = await postRequestKurye(email, password);
  ...
  async function postRequestKurye(email, password) {
    try {
      const postData = {
        id: email.value,
      ...
      const response = await axios.post(
        "http://eczanev2-dev.eu-central-1.elasticbeanstalk.com/api/postKuryeLogin",
        postData
      );
